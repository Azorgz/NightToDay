    ...
    def sky_veg_consistency(self, I, mask_seg):
        ...
        for b in range(I.size(0)):
            if sky_mask[b].sum() > 0:
                ...
                loss[b] += -((gradient_I_x[b] * sky_contour[..., 1:]) + (gradient_I_y[b] * sky_contour[..., 1:, :])).mean()
                loss[b] += erosion(sky_mask[b, :, 1:, 1:], kernel=torch.ones([3, 3])) * (gradient_I_x[b] ** 2 + gradient_I_y[b] ** 2).mean()
            if veg_mask[b].sum() > 0:
