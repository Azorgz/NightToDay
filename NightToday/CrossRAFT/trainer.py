...
class Trainer:
    ...
    def reload_checkpoint(self, root_path, epoch, model, optimizer, scheduler):
        model_checkpoint = torch.load(os.path.join(root_path,
                                                'model/checkpoints-%d.ckpt' % epoch))
        train_checkpoint = torch.load(os.path.join(root_path,
                                                'training/checkpoints-%d.ckpt' % epoch))
    ...
    def train_for_steps(self, total_steps = 1000):
        ...
        torch.save(checkpoint, os.path.join(self.model_checkpoint_path, 'checkpoints-%d.ckpt' % self.step))
        train_checkpoint = {
            'optimizer': self.optimizer.state_dict(),
            'step': self.step
        }
        torch.save(train_checkpoint, os.path.join(self.optimizer_checkpoint_path, 'checkpoints-%d.ckpt' % self.step))
